...
enum HTMLReporter {
    ...
    private static func renderFinding(_ finding: Finding, index: Int) -> String {
        let severityClass = "finding-\(finding.resolvedSeverity.rawValue)"
        let badgeClass    = "badge-\(finding.resolvedSeverity.rawValue)"
        let confClass     = finding.resolvedConfidence == .inferred ? "badge-inferred" : "badge-certain"
        let confLabel     = finding.resolvedConfidence.rawValue
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    details.raw-area[open] > summary {
        border-radius: var(--radius) var(--radius) 0 0;
        border-bottom: 1px solid var(--border);
    }

    details.raw-area > pre {
        padding: 1rem;
        font-size: .78rem;
        line-height: 1.55;
        overflow-x: auto;
